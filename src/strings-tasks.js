...
function removeFirstOccurrences(str, value) {
  const findValue = str.indexOf(value);

  if (findValue !== -1) {
    return str.slice(0, findValue) + str.slice(findValue + value.length);
  }
...
function removeLastOccurrences(str, value) {
  const findValue = str.lastIndexOf(value);
...
function findLongestWord(sentence) {
  ...
  newSentence.sort((a, b) => b.length - a.length);
...
function extractEmails(str) {
  const email = str.split(';').map((mail) => mail.trim());
  return email;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (newLetter) => {
    const char = newLetter <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    return String.fromCharCode(
      ((newLetter.charCodeAt(0) - char + 13) % 26) + char
    );
  });
}
